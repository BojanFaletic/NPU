...
class Plan:
    ...
    @classmethod
    def make(cls, M: int, K: int, N: int) -> "Plan":
        ...
        for cols in (8, 4, 2, 1):
            ...
            if K % k != 0:
                continue
            # Score: minimise total padded ops
            score = M_pad * N_pad
        ...
        assert best is not None, f"no valid plan for shape M={M} K={K} N={N} (K must be multiple of 64)"
        return best
