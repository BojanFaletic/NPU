"""Decode-time semi-compact IQ4_XS dequant+matvec on XDNA 2.

This format sits between raw IQ4_XS and fully-expanded bf16:
  - raw IQ4_XS:       136 bytes / 256 weights, slow on-tile nibble lookup
  - packed IQ4_XS:    288 bytes / 256 weights, int8 qvalues + fp32 scales
  - expanded bf16:    512 bytes / 256 weights, fastest tile MAC

The goal is to reduce routed down expert resolve time and resident memory while
avoiding the raw kernel's scalar nonlinear lookup on tile.
"""
...
from quant_mv_iq4 import BLK, RAW_B_BYTES
...
CACHE = ROOT / "build" / "quant_mv_iq4_packed_cache"
KERNEL_SRC = ROOT / "quant_mv_iq4_packed_kernel.cc"
...
def compile_kernel(m: int, build_dir: Path) -> Path:
    obj = build_dir / f"quant_mv_iq4_packed_{m}.o"
    ...
    print(f"[peano] compiling quant_mv_iq4_packed_{m}.o")
...
def generate_mlir(M: int, K: int, m: int, n_cores: int,
    ...
    zero = Kernel("zero_f32_qmv_iq4_packed", obj_name, [c_ty])
    matvec = Kernel("quant_mv_iq4_xs_packed_bf16_f32", obj_name, [a_ty, b_ty, c_ty])
...
def build_xclbin(M: int, K: int, m: int = 32, n_cores: int = 4) -> Compiled:
    ...
    tag = f"qmv_iq4_packed_{M_pad}x{K}_m{m}_c{n_cores}"
    build = CACHE / tag
...
def _repack_iq4_packed_raw(raw: np.ndarray, M: int, K: int) -> np.ndarray:
    K_blocks = K // BLK
    ...
    dl = d * scales.astype(np.float32)

    q_low = _KVALUES[qs & np.uint8(0x0F)]
    q_high = _KVALUES[qs >> np.uint8(4)]
    qvals = np.concatenate([q_low, q_high], axis=-1)

    out = np.empty((M, K_blocks, B_BYTES), dtype=np.uint8)
    out[..., :32] = dl.astype(np.float32).view(np.uint8).reshape(M, K_blocks, 32)
    out[..., 32:] = qvals.view(np.uint8).reshape(M, K_blocks, 256)
    return out.reshape(M, K_blocks * B_BYTES)


def repack_iq4_xs_packed_weight(t) -> np.ndarray:
    ...
    return _repack_iq4_packed_raw(raw, M, K)


def repack_iq4_xs_packed_per_expert(t, expert_idx: int) -> np.ndarray:
    from gguf import GGMLQuantizationType
    if t.tensor_type != GGMLQuantizationType.IQ4_XS:
        raise ValueError(f"need IQ4_XS tensor, got {t.tensor_type.name}")
    if len(t.shape) != 3:
...
class NpuIQ4PackedMatVec:
    """Single-vector F.linear(x, W) for semi-compact IQ4_XS weights."""
    ...
    @classmethod
    def from_iq4_xs_packed_bytes(cls, packed: np.ndarray, M: int, K: int,
                                 n_cores: int = 4,
                                 m: int | None = None) -> "NpuIQ4PackedMatVec":
    ...
    def from_gguf_tensor(cls, t, expert_idx: int | None = None,
        ...
        if expert_idx is None:
            packed = repack_iq4_xs_packed_weight(t)
            M = int(t.shape[1])
            K = int(t.shape[0])
        else:
            packed = repack_iq4_xs_packed_per_expert(t, expert_idx)
            M = int(t.shape[1])
            K = int(t.shape[0])
        return cls(packed, M, K, n_cores=n_cores, m=m)
